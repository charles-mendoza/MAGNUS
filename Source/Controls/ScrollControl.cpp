...
MAGNUS_CONTROL_CONSTRUCTOR(ScrollControl)
{
...
void ScrollControl::VBarMoved(Controls::Control* /*control*/)
{
	Invalidate();
}

void ScrollControl::HBarMoved(Controls::Control* /*control*/)
{
	Invalidate();
}

void ScrollControl::OnChildBoundsChanged(Magnus::Rect /*oldChildBounds*/, Control* /*pChild*/)
{
	UpdateScrollBars();
	Invalidate();
}

void ScrollControl::Layout(Magnus::Skin* skin)
{
	UpdateScrollBars();
...
{
	// Do not scroll if a menu is open
	/*for (Control::List::iterator iter = GetCanvas()->Children.begin(); iter != GetCanvas()->Children.end(); ++iter)
	{
		Control* pChild = *iter;
		Menu* pMenu = magnus_cast<Menu>(pChild);
		if (!pMenu) { continue; }
		if (pMenu->Visible()) { return false; }
	}*/
...
void ScrollControl::Render(Magnus::Skin* skin)
{
#if 0
	// Debug render - this shouldn't render ANYTHING REALLY - it should be up to the parent
	Magnus::Rect rect = GetRenderBounds();
	Magnus::Rendering::Renderer* render = skin->GetRenderer();
	render->SetDrawColor(Magnus::Color(100, 255, 255, 0));
	render->DrawFilledRect(rect);
	render->SetDrawColor(Magnus::Color(100, 255, 0, 0));
	render->DrawFilledRect(m_InnerPanel->GetBounds());
	render->RenderText(Utility::FormatUnicode(L"Offset: %i %i", m_InnerPanel->X(), m_InnerPanel->Y()), Magnus::Point(0, 0), skin->GetDefaultFont());
#else
	(void)skin;
#endif
}
...
{
	if (!m_InnerPanel) { return false; }

	for (Control::List::iterator iter = m_InnerPanel->Children.begin(); iter != m_InnerPanel->Children.end(); ++iter)
	{
		Control* pChild = *iter;

		if (pChild->GetDock() == Pos::None) { return false; }
	}
